...
def generate_test_suite_v2():
    ...
    return __generate_test_suite(
        2,
        [f.replace(".brewin", "") for f in os.listdir('./v2/tests') if f.endswith(".brewin")],
        [f.replace(".brewin", "") for f in os.listdir("./v2/fails") if f.endswith(".brewin")]
    )
...
def generate_test_suite_v3():
    ...
    return __generate_test_suite(
        3, 
        [f.replace('.brewin', '') for f in os.listdir('./v3/tests') if f.endswith('.brewin')], 
        [f.replace('.brewin', '') for f in os.listdir('./v3/fails') if f.endswith('.brewin')]
    )
